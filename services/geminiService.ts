...
export const generateCharacter = async (characterData: { concept: string; background: string }): Promise<{ character: Character; initialScene: Scene; introStory: string; }> => {
...
export const generateNextScene = async (character: Character, scene: Scene, history: StoryEntry[], playerAction: string): Promise<GameTurnResponse> => {
...
Tugas Anda:
1.  **Narasikan Hasilnya**: Deskripsikan apa yang terjadi selanjutnya secara hidup dan menarik (2-4 kalimat). Dunia ini berbahaya dan tindakan memiliki konsekuensi.
2.  **Tangani Aksi Ekonomi (Prioritas Utama)**: Jika aksi pemain adalah untuk membeli atau menjual, tangani ini SEBELUM hal lain.
    *   **Membeli**: Jika pemain berkata "beli [item] dari [NPC]", periksa: 1) Apakah NPC ada? 2) Apakah NPC menjual item itu? 3) Apakah pemain punya cukup 'gold'? Jika ya, kurangi emas pemain, tambahkan item ke inventaris pemain, dan narasikan transaksinya. Buat notifikasi (misal: "Item Diperoleh: Pedang Besi", "-50 Emas"). Jika gagal, jelaskan alasannya (tidak cukup uang, barang tidak ada).
    *   **Menjual**: Jika pemain berkata "jual [item] kepada [NPC]", periksa: 1) Apakah pemain punya item itu? 2) Apakah NPC mau membeli (seorang pandai besi membeli senjata, bukan buku sihir). Jika ya, hapus item dari pemain, tambahkan emas ke pemain (biasanya 50% dari 'value' item), dan narasikan. Buat notifikasi (misal: "+25 Emas"). Jika gagal, jelaskan alasannya.
3.  **Lakukan Skill Check (Jika Perlu)**: Jika aksi pemain hasilnya tidak pasti (DAN BUKAN AKSI EKONOMI), lakukan pemeriksaan keterampilan. Tentukan DC, lempar d20, hitung bonus, dan isi objek 'skillCheck'.
4.  **Perbarui Karakter**: Perbarui status karakter ('karakterTerbaru') berdasarkan hasil aksi. Ini termasuk HP, reputasi, item baru dari rampasan, dan perubahan emas.
5.  **Perbarui Adegan**: Deskripsikan lokasi baru jika pemain berpindah, atau perbarui NPC yang ada (sikap mereka, atau inventaris mereka setelah transaksi).
6.  **Buat Notifikasi**: Jika pemain menemukan item, emas, reputasinya berubah, atau peristiwa penting lainnya terjadi, tambahkan pesan singkat ke array 'notifications'.
7.  **Kondisi Kalah**: Jika HP karakter turun menjadi 0 atau di bawahnya, narasikan kematian mereka dengan dramatis.
