...
import { Character, ItemSlot, AnyItem, EquippableItem, InventoryItem, ItemRarity, Weapon, Armor, Accessory } from '../types';
        ...
        <div className="p-1 flex flex-col h-full">
            ...
            <div className="flex-grow flex flex-col min-h-0">
                <div>
                    ...
                    <ul className="space-y-2 text-sm overflow-y-auto flex-grow pr-1 max-h-[calc(100vh - 16rem)]">
                        {inventory.map((invItem) => (
                            <li key={invItem.item.id} className="bg-stone-950/40 p-2 rounded-md border border-stone-700/50 flex flex-col gap-2">
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-xs text-yellow-400 flex items-center gap-1 flex-shrink-0 ml-2"><CoinIcon className="w-3 h-3"/>{invItem.item.value}</span>
                                </div>
