...
const MarketplaceScreen: React.FC<MarketplaceScreenProps> = ({ marketplace, scene, character, onBuyItem, onSellItem, isLoading }) => {
    ...
    const availableShops = useMemo(() => {
        return marketplace.shops.filter(shop => scene.availableShopIds?.includes(shop.id));
    }, [marketplace, scene]);

    const activeShop = useMemo(() => {
        return marketplace.shops.find(shop => shop.id === activeShopId);
    }, [marketplace, activeShopId]);
    ...
    const renderShopInterface = (shop: Shop) => (
        <div>
            <div className="mb-4 text-center">
                ...
                <h4 className="font-cinzel text-lg text-amber-200">{shop.name}</h4>
                <p className="text-xs text-stone-400 italic">{shop.description}</p>
            </div>
            ...
            <ul className="space-y-2 text-sm overflow-y-auto max-h-[50vh] pr-1">
                {mode === 'buy' && (shop.inventory.map((item, index) => (
                    <li key={index} className="bg-stone-950/40 p-2 rounded-md border border-stone-700/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <button 
                            onClick={() => onBuyItem(item, shop.id)} 
                ...
                {mode === 'buy' && shop.inventory.length === 0 && <li className="text-stone-500 italic text-center py-4">Toko ini kehabisan stok.</li>}
                ...
                {mode === 'sell' && (character.inventory.map((item, index) => (
                    <li key={index} className="bg-stone-950/40 p-2 rounded-md border border-stone-700/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <button 
                            onClick={() => onSellItem(item, shop.id)}
                            disabled={isLoading}
    ...
    return (
        <div className="p-1 flex flex-col h-full">
            ...
            {availableShops.length > 0 && !activeShop && renderShopList()}
            {activeShop && renderShopInterface(activeShop)}
        </div>
    );
};
